...
var updateFlows = require('./updateFlows');
...
var NodeRED = AbstractComponent.extend({
            ...
            RED.start()
                ...
                .then(function () {
                    if (flows && flows.length > 0) {
                        // set flows using current value in dictionary
                        try {
                        ...
                        if (Object.prototype.toString.call(flows) === '[object Array]') {
                            RED.nodes.setFlows(flows)
                                ...
                                .then(function () {
                                    return updateFlows(RED, flows, this.inputWSS.port, this.outputWSS.port);
                                }.bind(this))
                                .then(function () {
                                    done();
                                })
                            ...
                            done(new Error('"'+this.getName()+'" attribute "flows" must be a JSON Array'));
                        }
                    } else {
                        this.startInputOutputServers()
                            .then(function () {
                                return updateFlows(RED, flows, this.inputWSS.port, this.outputWSS.port);
                            }.bind(this))
                            .then(function () {
    ...
    startInputOutputServers: function () {
        return findPort()
            .then(function (port) {
                ...
                this.log.debug(this.toString(), '"'+this.getName()+'" WebSocket kevoreeInputServer available at 127.0.0.1:'+port);
                return findPort();
            }.bind(this))
            .then(function (port) {
                this.outputWSS = createOutputServer(port);
                this.log.debug(this.toString(), '"'+this.getName()+'" WebSocket kevoreeOutputServer available at 127.0.0.1:'+port);
    ...
    in_input: function (data) {
        try {
            ...
            this.log.debug(this.toString(), '"'+this.getName()+'" incoming message sent to NodeRED through kevoreeInput');
        } catch (err) {
            this.log.warn(this.toString(), '"'+this.getName()+'" error sending message to NodeRED through kevoreeInput ('+err.message+'). Message lost');
        }
    },
